import Base: *, conj, copy, eltype, length, size, sortperm, transpose
...
function quadratic_operator(os::OpSum)
    ...
    for n in 1:nterms
        ...
        length(term) ≠ 2 &&
            error("Must create hopping Hamiltonian from quadratic Hamiltonian")
...
LinearAlgebra.Hermitian(A::T) where {T <: AbstractSymmetry} = set_data(A, Hermitian(A.data))
...
function correlation_matrix_to_mps(
    ...
    ns, C = correlation_matrix_to_gmps(
        Λ; eigval_cutoff = eigval_cutoff, minblocksize = minblocksize,
        maxblocksize = maxblocksize
    )
    ...
    elseif all(hastags("Electron"), s)
        ...
        if isspinful(s)
            if typeof(Λ_up) <: ConservesNf
                space_up =
                    [QN(("Nf", 0, -1), ("Sz", 0)) => 1, QN(("Nf", 1, -1), ("Sz", 1)) => 1]
                space_dn =
                    [QN(("Nf", 0, -1), ("Sz", 0)) => 1, QN(("Nf", 1, -1), ("Sz", -1)) => 1]
            ...
            elseif typeof(Λ_up) <: ConservesNfParity
                ...
                sf = siteinds(
                    "Fermion", N; conserve_qns = false, conserve_sz = false,
