...
@testset "Fermion (real and complex)" begin
    ...
    for θ in θs
        ...
        @test gmps * Λ * gmps' ≈ Diagonal(ns) rtol = 1.0e-2
        @test gmps' * Diagonal(ns) * gmps ≈ Λ rtol = 1.0e-2
        ...
        @test inner(ψ', H, ψ) ≈ E rtol = 1.0e-5
        @test inner(ψ', H, ψ) / norm(ψ) ≈ E rtol = 1.0e-5

        # Compare to DMRG
        sweeps = Sweeps(10)
        setmaxdim!(sweeps, 10, 20, 40, 60)
        setcutoff!(sweeps, 1.0e-12)
        energy, ψ̃ =
            dmrg(H, productMPS(s, n -> n ≤ Nf ? "1" : "0"), sweeps; outputlevel = 0)

        # Create an mps
        @test abs(inner(ψ, ψ̃)) ≈ 1 rtol = 1.0e-5
        @test inner(ψ̃', H, ψ̃) ≈ inner(ψ', H, ψ) rtol = 1.0e-5
        @test E ≈ energy
    end
end
...
@testset "Fermion BCS (real,real - no pairing, complex)" begin
    ...
    for (Delta, t) in zip(Deltas, ts)
        ...
        @test gmps * Λ.data * gmps' ≈ Diagonal(ns) rtol = 1.0e-2
        @test gmps' * Diagonal(ns) * gmps ≈ Λ.data rtol = 1.0e-2

        # Form the MPS
        s = siteinds("Fermion", N; conserve_qns = false)
        h_mpo = MPO(os_h + os_p, s)
        psi = correlation_matrix_to_mps(
            s, ElT.(c); eigval_cutoff = 1.0e-10, maxblocksize = 14, cutoff = 1.0e-11
        )
        ...
        @test E_dmrg ≈ E_ni_mpo rtol = 1.0e-4
        @test inner(psidmrg, psi) ≈ 1 rtol = 1.0e-4
