...
function expect_compat(psi::MPS, ops::AbstractString...; kwargs...)
    ...
    for j in site_range
        ...
        for n in 1:Nops
            ex[n][j - offset] =
                real(scalar(psi[j] * op(ops[n], s[j]) * dag(prime(psi[j], s[j]))))
        end
    end
...
@testset "Electron" begin
    ...
    @test tr(Φ_up' * h_up * Φ_up) + tr(Φ_dn' * h_dn * Φ_dn) ≈
        inner(ψ0', H_noninteracting, ψ0) rtol =
        1.0e-3
    ...
    setnoise!(sweeps, 1.0e-5, 1.0e-6, 1.0e-7, 0.0)
...
@testset "Electron - Pairing (currently inactive)" begin
    ...
    if !is_implemented
        ...
        # Create an MPS from the slater determinants.
        s = siteinds(
            "Electron", N; conserve_qns = false, conserve_nfparity = true,
            conserve_nf = false
        )
        H_ni_up = MPO(os_up, s)

        ψ0 = slater_determinant_to_mps(
            s, Φ_up, Φ_dn; eigval_cutoff = 1.0e-4, cutoff = _cutoff,
            maxdim = _maxlinkdim
        )
        @show norm(ψ0)
        @test maxlinkdim(ψ0) ≤ _maxlinkdim
        ...
        @show tr(Φ_up' * h_up * Φ_up),
            ...
            inner(ψ0', H_ni_up, ψ0)
        @test tr(Φ_up' * h_up * Φ_up) + tr(Φ_dn' * h_dn * Φ_dn) ≈
            inner(ψ0', H_noninteracting, ψ0) rtol = 1.0e-3
